...
const diyFileCheck = (target_file) => {
  ...
  let dir = `./src/widgets/${project_category_name}/views/example/${target_file}`
  const files = glob.sync(dir,{ 'nodir': true})
  if(files.length === 1) {
    plugins.push(new copy([{from: `./src/widgets/${project_category_name}/views/example/setting.json`, to: `./`}]))
  }
}
diyFileCheck()
